...
function getOrdersTotal(orderList) {
  return orderList.map(order => order.price * (order.tax + 1))
}

ordersTotal = getOrdersTotal(orders);
...
let bobsTotal = purchases.filter(obj => obj.owner === 'Bob').reduce((a, c) => a.price === undefined ? a + c.price : a.price + c.price);
